...
function getRequest3() {
    let url = "/api/cas?page=2&pagesize=20";
...
function afficheRestaurantsEnTable(cas) {
    ...
    for(var i=0; i < cas.length; i++) {
        ...
        let restaurant = cas[i];
        let nom = restaurant.id_cas;
        let cuisine = restaurant.nom_dossier;

        let celluleNom = ligne.insertCell();
        celluleNom.innerHTML = nom;
        celluleNom.id = "cas" + i + "Nom" ;

        let celluleCuisine = ligne.insertCell();
        celluleCuisine.innerHTML = cuisine;
        celluleCuisine.id = "cas" + i + "Cuisine" ;

        let celluleRemove = ligne.insertCell();
        celluleRemove.innerHTML = '<button id=' + restaurant._id + ' onclick="supprimerCas(event);">Supprimer</button>';

        let celluleModifier = ligne.insertCell();
        celluleModifier.innerHTML = '<button id=' + restaurant._id + ' onclick="modifierCas(' + i + ');">Modifier</button>';
